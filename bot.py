...
# Класс для работы с Crypto Bot API (исправленная версия)
class CryptoBotAPI:
    ...
    async def create_invoice(self, amount, currency, description=""):
        ...
        # Правильный формат для Crypto Bot API
        payload = {
            "asset": currency.upper(),  # Используем asset вместо currency_type
            "amount": str(amount),
            "description": description[:1024],  # Ограничиваем длину описания
...
async def pay_with_crypto(callback: CallbackQuery):
    ...
    # Конвертация в криптовалюту с округлением
    if currency == "USDT":
        crypto_amount = round(price_rub / 90, 2)
        # Минимальная сумма для USDT в Crypto Bot
        if crypto_amount < 0.1:
            crypto_amount = 0.1
    else:  # TON
        crypto_amount = round(price_rub / 120, 2)
        # Минимальная сумма для TON в Crypto Bot
        if crypto_amount < 0.1:
            crypto_amount = 0.1
